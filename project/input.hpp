...
public:
    ...
    void AddUpdateKeyCallback(KeySeq const &sequence, OnUpdateKeyCallbackFn const &fn) { on_update_callbacks_[sequence] = std::pair{0u, fn}; }
    void AddTickKeyCallback(KeySeq const &sequence, OnTickKeyCallbackFn const &fn) { on_tick_callbacks_[sequence] = std::pair{0u, fn};}
...
private:
    ...
    std::map<KeySeq, std::pair<uint32_t, OnTickKeyCallbackFn>> on_tick_callbacks_;
    std::map<KeySeq, std::pair<uint32_t, OnUpdateKeyCallbackFn>> on_update_callbacks_;
};