...
cbuffer OpaquePerMaterial : register(b2)
{
    ...
    float3 reflective_color;
    float roughness_value;
    float reflectance_value;
    uint enabled_texture_flags;
    float2 uv_multiplier;
};
...
float3 ComputePointLightsEnergy(PBR_Material material, PBR_CommonData common_data)
{
    ...
    {
        rv += ComputePointLightEnergy(material, common_data, point_lights[i]);
    }
    return rv;
}
    ...
    : SV_TARGET
{
    ...
    input.texcoord = input.texcoord * uv_multiplier;
    if (enabled_texture_flags & TEXTURE_ENABLED_AMBIENT)
    {
        ambient += g_ambient.Sample(g_default_sampler, input.texcoord).rgb;
    }
    ambient += ambient_color;
    PBR_Material material;
    if (enabled_texture_flags & TEXTURE_ENABLED_ALBEDO)
    {
    ...
    if (enabled_texture_flags & TEXTURE_ENABLED_ROUGHNESS)
    {
    ...
    if (enabled_texture_flags & TEXTURE_ENABLED_METALNESS)
    {
