...
uint2 stream_random_uint(inout uint2 state, in uint iterations = 1,
                         in uint seed = 0x9E3779B9) {
  uint s = seed;
  for (uint i = 0; i < iterations; i++) {
    state.x += ((state.y << 4) + 0xA341316C) ^ (state.y + s) ^
               ((state.y >> 5) + 0xC8013EA4);
    state.y += ((state.x << 4) + 0xAD90777D) ^ (state.x + s) ^
               ((state.x >> 5) + 0x7E95761E);
    s += seed;
  }
  return state;
}

float2 stream_random_unit_float(inout uint2 state, in uint iterations = 1,
                                in uint seed = 0x9E3779B9) {
