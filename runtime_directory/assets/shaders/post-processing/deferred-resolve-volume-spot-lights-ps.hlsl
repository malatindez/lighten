...
struct PS_INPUT
{
  ...
  nointerpolation float3 position : LIGHT_POSITION;
...
float4 ps_main(PS_INPUT input) : SV_Target
{
  ...
  point_light.position = input.position;
  point_light.cone_direction = input.direction;
  point_light.inner_cutoff = input.inner_cutoff;
  point_light.outer_cutoff = input.outer_cutoff;
  ...
  {
    if(shadowed_spot(common_data, g_shadow_spot_lights[input.shadow_map_index], input.shadow_map_index))
      {
  ...
  output.rgb += ComputeSpotLightEnergy(material, common_data, point_light);
