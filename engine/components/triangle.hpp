...
namespace engine::components
{
    ...
    public:
        ...
        bool CheckIntersection(Transform const &, math::Intersection &i, math::Ray const &ray) const noexcept
        {
            ...
            float t = math::dot(points_[0] - ray.origin(), normal_) / ndotdir;
            if (t > i.t || t < 0)
            {
                return false;
            }
            ...
            math::vec3 c;
            math::vec3 edge0 = points_[1] - points_[0];
            math::vec3 vp0 = P - points_[0];
            c = math::cross(edge0, vp0);
            if (math::dot(normal_, c) < 0.0f)
            {
                return false;
            }
            math::vec3 edge1 = points_[2] - points_[1];
            math::vec3 vp1 = P - points_[1];
            c = math::cross(edge1, vp1);
            if (math::dot(normal_, c) < 0.0f)
            {
                return false;
            }
            math::vec3 edge2 = points_[0] - points_[2];
            math::vec3 vp2 = P - points_[2];
