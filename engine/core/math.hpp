...
namespace cereal {
    ...
    void serialize(Archive &archive, glm::mat<size_x, size_y, matrix_type, Q> &m)
    {
        for (glm::length_t i = 0; i < size_x; i++)
            for (glm::length_t j = 0; j < size_y; j++)
                archive(m[i][j]);
    }
}
...
namespace engine::core::math
{
    ...
    void invert_orthonormal(glm::mat<4, 4, T> const &src, glm::mat<4, 4, T> &dst)
    {
        ...
        dst[1][2] = src[2][1];
        dst[2][1] = src[1][2];
        glm::vec3 vec = -src[3].x * glm::vec3{dst[0][0], dst[0][1], dst[0][2]} - src[3].y * glm::vec3{dst[1][0], dst[1][1], dst[1][2]} - src[3].z * glm::vec3{dst[2][0], dst[2][1], dst[2][2]};
        dst[3][0] = vec.x;
        dst[3][1] = vec.y;
        dst[3][2] = vec.z;
        dst[0][3] = 0;
    ...
    template <typename T>
    void invert_orthogonal(glm::mat<4, 4, T> const &src, glm::mat<4, 4, T> &dst)
    {
        assert(&src != &dst);
        dst[0][0] = src[0][0];
        dst[1][1] = src[1][1];
        ...
        dst[3][0] =
            -src[3].x * dst[0][0] / lengths[0] - src[3].x * dst[0][1] / lengths[0] - src[3].x * dst[0][2] / lengths[0];
        dst[3][1] =
            -src[3].y * dst[1][0] / lengths[1] - src[3].y * dst[1][1] / lengths[1] - src[3].y * dst[1][2] / lengths[1];
        dst[3][2] =
            -src[3].z * dst[2][0] / lengths[2] - src[3].z * dst[2][1] / lengths[2] - src[3].z * dst[2][2] / lengths[2];
        dst[0][3] = 0;
        dst[1][3] = 0;
        dst[2][3] = 0;
        dst[3][3] = 1;
    ...
    template <typename T>
    glm::mat<4, 4, T> invert_orthonormal(glm::mat<4, 4, T> const &src)
    {
        glm::mat<4, 4, T> return_value;
        invert_orthonormal(src, return_value);
        return return_value;
    }
    template <typename T>
    glm::mat<4, 4, T> invert_orthogonal(glm::mat<4, 4, T> const &src)
    {
        glm::mat<4, 4, T> return_value;
