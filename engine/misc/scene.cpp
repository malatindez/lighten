...
namespace engine
{
    void Scene::SetBitmapWindow(std::shared_ptr<BitmapWindow> window)
    {
        window_ = window;
    }

    [[nodiscard]] math::Sphere &Scene::sphere() noexcept
    {
        return sphere_;
    }
    ...
    bool Scene::Update(float)
    {
        if (window_ == nullptr)
        {
            return false; // stop updating if window is destroyed
        }
        Draw();
        return true;
    }
